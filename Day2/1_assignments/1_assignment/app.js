...
// Code to send SMS
a1.on('depositSuccess', (balance) => {
    smsClient.send(`Amount Deposited - New Account Balance is: INR ${balance}`);
});

a1.on('withdrawSuccess', (balance) => {
    smsClient.send(`Amount Withdrawn - New Account Balance is: INR ${balance}`);
});
...
// Code to send Email
a1.on('depositSuccess', (balance) => {
    emailClient.send(`Amount Deposited - New Account Balance is: INR ${balance}`);
});

a1.on('withdrawSuccess', (balance) => {
    emailClient.send(`Amount Withdrawn - New Account Balance is: INR ${balance}`);
});
